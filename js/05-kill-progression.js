...
function classicDropMult() { return player.classicMode ? 0.1 : 10; }   // 🎮 經典模式：所有物品掉落機率 ×1/10
...
function classicExemptDropMult(id) {
    if (typeof TRIAL_ITEM_CLASS !== 'undefined' && TRIAL_ITEM_CLASS[id]) return 10;
    if (isRelic(DB.items[id])) return 10;
...
function killMob(idx) {
    ...
    let _dropBase = (mob._grace ? 10 : (mob._sherine ? (mob._sherineMad ? 5 : 3) : 10));   // 🔮 席琳的世界 ×3（瘋狂×5）／恩賜怪 ×10（不含經典 ×1/10，供試煉道具用）
